...
var board = new firmata.Board("/dev/ttyACM0", function(){
    ...
    board.pinMode(13, board.MODES.OUTPUT); //Inicializiramo izhode za lučke
    board.pinMode(12, board.MODES.OUTPUT);
    board.pinMode(8, board.MODES.OUTPUT);
});

function handler(req, res) {
    fs.readFile(__dirname + "/primer12.html",
    function (err, data) {
        if (err) {
...
board.on("ready", function() {
    ...
    board.analogRead(1, function(value){
        ...
        // V nadaljevanju določimo logične pogoje za pravilno delovanje kontrolnih lučk
        if (dejanskaVrednost < želenaVrednost) {
